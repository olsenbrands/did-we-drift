...
- The primary holds plan-lane (SSOT/trackers/decision records → cited claims of what should
  be happening) and the D1–D6 scoring (mechanical, no bias risk, never a separate seat), then
  diffs the two accounts and writes the report.
...
- Two lane workers: plan-lane (reads only SSOT/trackers/decision records) and work-lane
  (as above, plan-blind).
- One **blind synthesis seat**: receives the two lane reports VERBATIM plus the deliverable
  sentence and stage list — never the primary's hypothesis, draft mapping, correction ideas,
  or the lane identities. It may open a cited source only to resolve a stated conflict
  between the lanes. Output: verdict + grade + smallest correction.
...
The verdict still comes out in SKILL.md §4's report shape, stamped per §5, with
`Auditor: cross-model` or `(same model, independent context)` stated honestly.
